...
class DFSEO_Settings {
	...
	const GROUPS = [
		...
		'dfseo_advanced' => [ 'dfseo_noindex_archives', 'dfseo_noindex_author', 'dfseo_noindex_tags', 'dfseo_noindex_paged', 'dfseo_breadcrumbs_enable', 'dfseo_breadcrumbs_separator', 'dfseo_breadcrumbs_home_label', 'dfseo_robots_txt_custom', 'dfseo_track_404', 'dfseo_redirect_enable' ],
	...
	public static function handle_save( string $group ): bool {
		...
		foreach ( $options as $key ) {
			if ( ! isset( $_POST[ $key ] ) ) {
				// Checkbox / array — delete if not present
				delete_option( $key );
				continue;
			}
			...
			} elseif ( $key === 'dfseo_robots_txt_custom' ) {
